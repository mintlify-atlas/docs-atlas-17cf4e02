...
Astro is an all-in-one web framework. It includes everything you need to create a website, built-in. There are also hundreds of different [integrations](https://astro.build/integrations/) and API hooks available to customize a project to your exact use case and needs.
...
<CardGroup cols={2}>
  ...
  <Card title="Server-first" icon="server">
    Moves expensive rendering off of your visitors' devices.
  </Card>
  
  <Card title="Zero JS by default" icon="zap">
    Less client-side JavaScript to slow your site down.
  </Card>
  ...
  <Card title="Customizable" icon="puzzle">
    Partytown, MDX, and hundreds of integrations to choose from.
  </Card>
</CardGroup>
...
<Note>
Astro's server-first rendering, either pre-rendered or on-demand, provides performant defaults that you can enhance and extend.
</Note>
...
Astro's magic is in how it combines the two values explained above — a content focus with a server-first architecture — to make tradeoffs and deliver features that other frameworks cannot. The result is amazing web performance for every website, out of the box. 
...
<Note>
One of our favorite sayings is **opt in to complexity**. We designed Astro to remove as much "required complexity" as possible from the developer experience, especially as you onboard for the first time.
</Note>
...
Astro invests in developer tools like:

- A great CLI experience from the moment you open your terminal
- An official VS Code extension for syntax highlighting, TypeScript, and Intellisense
- Documentation actively maintained by hundreds of community contributors and available in 14 languages
